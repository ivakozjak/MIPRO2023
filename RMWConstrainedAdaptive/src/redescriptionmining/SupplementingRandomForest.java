...
public class SupplementingRandomForest {
    public static void main(String[] args) throws IOException {
        ...
        File file = new File("logovi.txt");
        file.createNewFile();
        FileWriter writer = new FileWriter("logovi.txt");
        
        writer.write("\nNum targets: "+appset.numTargets);
        writer.write("\nNum trees in RS: "+appset.numTreesinForest);
        writer.write("\nAverage tree depth in RS: "+appset.aTreeDepth);
        writer.write("\nAllow left side rule negation: "+appset.leftNegation);
        writer.write("\nAllow right side rule negation: "+appset.rightNegation);
        writer.write("\nAllow left side rule disjunction: "+appset.leftDisjunction);
        writer.write("\nAllow right side rule disjunction: "+appset.rightDisjunction);
        writer.write("\nTypes of LSTrees: "+appset.treeTypes.get(0));
        writer.write("\nTypes of RSTrees: "+appset.treeTypes.get(1));
        writer.write("\nUse Network information: "+appset.useNC.toString());
        writer.write("\nSpatial matrix: "+appset.spatialMatrix.toString());
        writer.write("\nSpatial measure: "+appset.spatialMeasures.toString());
        writer.write("\nJSType: "+appset.jsType);
        writer.write("\nAttribute importance: "+appset.attributeImportance);
        ...
        if(appset.redesSetSizeType==1 && appset.numRetRed!=Integer.MAX_VALUE)
         ...
         writer.write("\nNumber of redescriptions: "+appset.numInitial);
        ...
        writer.write("\nWIndexes size: "+datJ.W2indexs.size());
       ...
       /* SettingsReader initSettings=new SettingsReader();
        ...
        writer.write("\ndistance file size: "+appset.distanceFilePaths.size()+"");
   ...
   */
           ...
           // writer.write("\nCuttof: "+minCovElements[0]);
           // writer.write("\nCuttof1: "+minCovElements1[0]);
         ...
         try{
             if(appset.treeTypes.get(0)==1/*appset.typeOfLSTrees==1*/)
                 if(appset.system.equals("windows"))
                     ...
                     dsc1.modifyDatasetS(startIndexRR,endTmp,rr,appset.outFolderPath+"/Jinputnew1.arff",fid,appset); //dsc1.modifyDatasetS(nARules1, startPerc, endPerc, oldIndexRR, endIndexRR, minCovElements[0],maxCovElements[0] ,rr,appset.outFolderPath+"\\Jinputnew1.arff"/*"C:\\Users\\matej\\Documents\\Redescription mining with CLUS\\UNCTADall3testnew1.arff"*/,fid);
       ...
       // rr.extractRules(input);
        ...
        writer.write("\nNew rules cicle 1: "+newRules);
        writer.write("\nNew rules cicle 2: "+newRules1);
       }
        ...
        if(appset.useJoin){
            ...
            newRedescriptions=rs.createGuidedNoJoinBasic(rr1, rr, jsN, appset, oldIndexRR, oldIndexRR1, RunInd,oom,fid,datJ, elemFreq, attrFreq, redScores,redScoresAtt,redDistCoverage,redDistCoverageAt, redDistNetwork, targetAtScore, Statistics, maxDiffScoreDistribution,nclMatInit,0);
            rr.removeElements(rr.newRuleIndex);
            rr1.removeElements(rr1.newRuleIndex);
            if(appset.numSupplementTrees>0){
                rr.removeRulesCF();
        ...
        writer.write("\nNew redescriptions: "+newRedescriptions);
        ...
        writer.write("\nNumber of viewes: "+datJ.W2indexs.size());
        ...
        for(int z=0;z<numBins/*percentage.length-1*/;z++){
           ...
           /* cuttof=rs.findCutoff(naex, startPerc, endPerc, minCovElements,maxCovElements,oldRIndex, appset.minSupport,appset.maxSupport,appset.numTargets);
             writer.write("\nminCovElements: "+minCovElements[0]);
             writer.write("\nmaxCovElements: "+maxCovElements[0]);
             writer.write("\ncuttof: "+cuttof);
        ...
        RunInd++;
        writer.write("\nRunning index: "+RunInd);
        }
      }
       
        writer.write("\nRedescription size main: "+rs.redescriptions.size());
        
        //removing all redescriptions with inadequate minSupport and minJS
        rs.remove(appset);
         
        writer.write("\nRedescription size main after remove: "+rs.redescriptions.size());
        
        //filtering
      // rs.filter(appset, rr, rr1,fid,datJ); // think about what we want and if we need it
        
        writer.write("\nRedescription size main after filter: "+rs.redescriptions.size());
      ...
      int numFullRed=0;
        ...
        rs.removePVal(appset);
         ...
         writer.write("\nRedescription size main after Pvalremove: "+rs.redescriptions.size());
        
        writer.write("\nFound "+numFullRed+" redescriptions with JS=1.0 and minsupport>"+appset.minSupport);
        writer.write("\nFound "+rs.redescriptions.size()+" redescriptions with JS>"+appset.minJS);
         ...
         rs.adaptSet(datJ, fid,0);
          writer.write(rs.redescriptions.size()+" redescriptions with JS>"+appset.minJS+" left after elimination of long rules");
       ...
       //nclMatInit.loadGlobalInfo(appset);
        ...
        if(appset.attributeImportance>0){
            for (int i = rs.redescriptions.size() - 1; i >= 0; i--)
                if (rs.redescriptions.get(i).checkAttributes(appset, fid, datJ) == 0)
                    rs.redescriptions.remove(i);
        }

        if(appset.elementImportance>0){
            for (int i = rs.redescriptions.size() - 1; i >= 0; i--)
                if (rs.redescriptions.get(i).checkElements(appset, fid, datJ) == 0)
                    rs.redescriptions.remove(i);
        }
       ...
       writer.write("\nComputing rule score and sorting!");
      ...
      //rs.writeToFile(appset.outFolderPath+"\\"+appset.outputName, datJ, fid, rr, rr1, startTime,numFullRed,appset);//fix output file name
       //writer.write("\nnum nodes global: "+nclMatInit.globalNodeInfo.keySet().size());
     if(appset.system.equals("windows"))
     ...
     if( appset.spatialMatrix.size()<=0 ) {
         ...
         if (appset.system.equals("windows"))
             Result.writePlotsNet(appset.outFolderPath + "\\" + "RuleData" + (1) + ".csv", appset, datJ, nclMatInit, fid);
         else
             Result.writePlotsNet(appset.outFolderPath + "/" + "RuleData" + (1) + ".csv", appset, datJ, nclMatInit, fid);
     }
