...
public class Redescription implements Comparable<Redescription>{
    ...
    int checkElements(ApplicationSettings appset, Mappings map, DataSetCreator dat){

        int contains=0, notContained=0;
        System.out.println("elems: " + elements);

        for(int i=0;i<appset.importantElements.size();i++)
            if(elements.contains(map.exampleId.get(appset.importantElements.get(i))) && appset.elementImportance==1){
                contains=1;
                return contains;
            }
            else if(!elements.contains(map.exampleId.get(appset.importantElements.get(i))) && appset.elementImportance==2){
                notContained=1;
            }

        if(appset.elementImportance == 2){
            if(notContained==1)
                contains=0;
            else contains=1;
        }
