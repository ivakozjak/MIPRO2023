...
redApp.controller('redCtrl', function ($scope, $route, $http,$timeout,$interval,$q,$location,$anchorScroll) {
            ...
            $http(options)
                .success(function (data) {
                    ...
                    for (i=0; i<data.redescriptionsCountCLUS; i++){
                        $scope.redescriptionTable[i] = {redescription:$scope.redescriptionsCLUS[i], maxAEJs:$scope.maxAEJs[i], attJac:$scope.attJac[i]};
                    }
        ...
        if($scope.conditionElements && $scope.conditionw1selection && $scope.conditionw2selection) {
            ...
            $http(options)
                .success(function (data) {
                    ...
                    $scope.redescriptionTable = [];
                    for (i=0; i<data.redescriptionsCountCLUS; i++){
                        $scope.redescriptionTable[i] = {redescription:$scope.redescriptionsCLUS[i], EJsel:$scope.EJsel[i], maxAEJs:$scope.maxAEJs[i], attJac:$scope.attJac[i]};
                    }
                    if($scope.redescriptionsCLUS.length==0) {
                        delete $scope.redescriptionsCLUS;
                        $scope.noRedescriptionsfound = 1;
    ...
    function removeElementsAttrs() {
        for(i=0; i<$scope.elementsDescriptionSelected.length; i++) {
            id = "#" + $scope.elementsDescriptionSelected[i];
            if ( $(id).hasClass("match") ) {
                $(id).removeClass("match");
                $(id).addClass("noMatch");
            }
    ...
    $scope.showAttrs = function (){
        ...
        for(var i=0; i<$scope.attributeDescr[id-1].length; i++)
            $scope.descriptionAndName[i] = {attrName:$scope.redescriptionsCLUS[id-1].attrs[i], attrDescription:$scope.attributeDescr[id-1][i]};
        ...
        if($scope.loadedShowElem!=0){
            ...
            for(i=0; i<$scope.descriptions.length; i++) {
                ...
                for(j=0; j<$scope.elementsDescriptionSelected.length; j++) {
                    if($scope.elementsSelected[j]==$scope.descriptions[i])
                        id = "#" + $scope.elementsDescriptionSelected[j];
                }
                console.log($scope.descriptions[i] + id);
                                ...
                                if(typeof $scope.$parent.graphInput == 'undefined'){
                                    ...
                                    promiseNR.success(function (data) {
                                        ...
                                        joinedPromiseE.then(function(values){
                                            ...
                                            console.log('indAAJ'); console.log(indAAJ);
                                            console.log('indAEJ'); console.log(indAEJ);
                                            ...
                                            for(var vi=0;vi<tmp.length;vi++){
                                                ...
                                                sum = sum/num;
                                                sumAt = sumAt/num;
                                                console.log('sum '+sum); console.log('sumAt '+sumAt);
    ...
    $scope.trainSom = function (params) {//functions to be defined on a button press
        ...
        if(params!=null){
            ...
            var timer6=$timeout(function(){
                ...
                som = new Som({features:  feat, iterationCount: params.nIter/*elemIds.size()*/,initialLearningRate: params.lRate, width: params.nCols, height: params.nRows});
                ...
                function SOMIteration() {
                    ...
                    if(q<params.nIter){
                        ...
                        if(q==(elemIds.size()-1))
                            $scope.progress="100 % complete!";  var tt; $scope.checked = false; $scope.action=tt;
                    }
                    else {
                        if(q==params.nIter)
		...
		console.log($scope.SOMInput);},2000);*/
            ...
            var timer6=$timeout(function(){
                ...
                if($scope.action.type == "load"){
                    ...
                    var options1 = {url: "http://127.0.0.1:8089/phenotype/redescriptionElement",
                        method: 'GET',
                        params: {
                        }
                    };
