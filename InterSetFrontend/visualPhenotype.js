...
// Loading data
redApp.controller('redCtrl', function ($scope, $route, $http, $timeout, $interval, $q, $location, $anchorScroll) {

    $scope.init = function () {
        if (sessionStorage.getItem('username') !== null) {
            $("#loginForm").hide();
            var loginLink = "http://127.0.0.1:8089/phenotype/login";
    ...
    $scope.selectionNumber = 0;
    $scope.selectedCLUSRM = {clus: new Array()};
    ...
    $scope.actionSSel = {};
    $scope.actionSSel.type = 'cluster';
    $scope.buttonText = "Show settings";
    $scope.loadedElem = 0;
    $scope.loadedAttr = 0;
    $scope.loadedShow = 0;
    $scope.loadedShowElem = 0;
    $scope.attributesSelected = [];
    ...
    $scope.onTabSelected = function (tab) {
        ...
        if (typeof tab === 'string') {
            switch (tab) {
                case 'elementInfo':
                    route = 'elementInfoPhenotype.html';
                    break;
                case 'attributeInfo':
                    route = 'attributeInfoPhenotype.html';
                    break;
                case 'globalInfo':
                    route = 'globalInfoPhenotype.html';
                    break;
                default:
                    route = tab;
                    break;
    ...
    $scope.tabs = [{
        ...
        content: 'elementInfoPhenotype.html'
    }, {
        slug: 'attributeInfo',
        title: "Attribute - based exploration",
        content: 'attributeInfoPhenotype.html'
    }, {
        slug: 'globalInfo',
        title: "Property - based exploration",
        content: 'globalInfoPhenotype.html'
    }];
    ...
    $scope.getRedescriptonsInSOMCluster = function (options) {
        $http(options)
            .success(function (data) {
                delete $scope.selSOMElements;
                delete $scope.redescriptionsCLUS;
                delete $scope.descriptions;
                delete $scope.showSettings;
                initSettings();
                // attach this data to the scope
                $scope.RedsInSOMClust = data;
                $scope.$parent.riscG.RedsInSOMClust = data;
                $scope.error = '';
                if ($scope.loadedElem == 1) {
                    $scope.loadedShow = 1;
                    $scope.attributesSelected = data.attributesSelected;
                } else $scope.loadedShow = 0;
            })
            .error(function (data, status) {
                if (status === 404) {
                    $scope.error = 'REST server is offline!';
    ...
    $scope.compHeatmap = function (params) {//function to load and create heatmap on full data
        delete $scope.RedsContainingAttrs;
        delete $scope.noRedescriptionsfound;
        delete $scope.redescriptionsCLUS;
        delete $scope.showSettings;
        delete $scope.descriptionAndName;
        $scope.selectedCLUSRM.clus = [];
        $scope.loadedShowElem = 0;
        $scope.loadedAttr = 0;
        /*scope.attrURL=attrs.redattrUrl;
		scope.attrFreqURL=attrs.attrfreqUrl;
		scope.attrcoocURL=attrs.allredUrl;*/
        //$scope.selectedCLUSRM = "";
        $scope.selectionNumber = 0;
        ...
        $q.all([encapsulateHD(options)]).then(function () { //compute attribute frequency and return to database
                ...
                $q.all([encapsulateAttrFreqW(options1)]).then(function () {
                    console.log('Ready to draw heatmap!');
    ...
    $scope.runCLUSRMSettings = function () {
        if ($scope.conditionw1 == "none") console.log("Selected condition is NONE.");
        else if ($scope.conditionw1 == "soft") console.log("Selected condition is SOFT.");
        else if ($scope.conditionw1 == "hard") console.log("Selected condition is HARD.");
        else window.alert('Select condition for w1! SOFT/HARD');
        if ($scope.conditionw2 == "none") console.log("Selected condition is NONE.");
        else if ($scope.conditionw2 == "soft") console.log("Selected condition is SOFT.");
        else if ($scope.conditionw2 == "hard") console.log("Selected condition is HARD.");
        else window.alert('Select condition for w2! SOFT/HARD');
        if ($scope.selectedCLUSRM.clus.length == 0) window.alert('Select pair of attributes on heatmap!');
        if ($scope.loadedShowElem == 1) {
            removeElementsAttrs();
            if ($scope.conditionElementsSelection == "none") console.log("Selected condition is NONE.");
            else if ($scope.conditionElementsSelection == "soft") console.log("Selected condition is SOFT.");
            else if ($scope.conditionElementsSelection == "hard") console.log("Selected condition is HARD.");
            else window.alert('Select condition for elements!');
        } else {
            $scope.conditionElementsSelection = "none";
            $scope.elementsSelected = [];
        }
        if ($scope.conditionw1 && $scope.conditionw2 && $scope.conditionElementsSelection) {
            var clusLink = "http://127.0.0.1:8089/phenotype/clusrmSettings";
            var options = {
                url: clusLink,
                method: 'POST',
                params: {
                    conditionw1: JSON.stringify($scope.conditionw1),
                    conditionw2: JSON.stringify($scope.conditionw2),
                    selected: JSON.stringify($scope.selectedCLUSRM.clus),
            ...
            $http(options)
                .success(function (data) {
                    ...
                    console.log(data.redescriptionsCountCLUS);
                    console.log(data.redescriptionsCLUS);
                    console.log(data.attributeDescr);
                    console.log(data.listOfAttributes);
                    $scope.attributeDescr = data.attributeDescr;
                    $scope.listOfAttributes = data.listOfAttributes;
                    $scope.redescriptionsCLUS = data.redescriptionsCLUS;
                    //$scope.selectedCLUSRM = data.selected;
                    $scope.maxAEJs = data.maxAEJs;
                    $scope.attJac = data.attJac;
                    $scope.AEJs = data.AEJs;
                    $scope.AAJs = data.AAJs;
                    $scope.sumAEJ = data.sumAEJ;
                    $scope.sumAAJ = data.sumAAJ;
                    $scope.redescriptionTable = [];
                    for (i = 0; i < data.redescriptionsCountCLUS; i++) {
                        $scope.redescriptionTable[i] = {
                            redescription: $scope.redescriptionsCLUS[i],
                            maxAEJs: $scope.maxAEJs[i],
                            attJac: $scope.attJac[i]
                        };
                    }
                    if ($scope.redescriptionsCLUS.length == 0) {
                        delete $scope.redescriptionsCLUS;
                        $scope.noRedescriptionsfound = 1;
                    }
                })
                .error(function (data, status) {
                    if (status === 404) {
    ...
    $scope.runCLUSRMSettingsElements = function () {
        removeElements();
        if ($scope.conditionElements == "none") console.log("Selected condition is NONE.");
        else if ($scope.conditionElements == "soft") console.log("Selected condition is SOFT.");
        else if ($scope.conditionElements == "hard") console.log("Selected condition is HARD.");
        else {
            window.alert('Select condition for elements!');
            //window.location.reload();
        }
        if ($scope.loadedShow == 1) {
            if ($scope.conditionw1selection == "none") console.log("Selected condition is NONE.");
            else if ($scope.conditionw1selection == "soft") console.log("Selected condition is SOFT.");
            else if ($scope.conditionw1selection == "hard") console.log("Selected condition is HARD.");
            else window.alert('Select condition for w1! SOFT/HARD');
            if ($scope.conditionw2selection == "none") console.log("Selected condition is NONE.");
            else if ($scope.conditionw2selection == "soft") console.log("Selected condition is SOFT.");
            else if ($scope.conditionw2selection == "hard") console.log("Selected condition is HARD.");
            else window.alert('Select condition for w2!');
        } else {
            $scope.conditionw1selection = "none";
            $scope.conditionw2selection = "none";
            $scope.attributesSelected = [];
        }
        if ($scope.conditionElements && $scope.conditionw1selection && $scope.conditionw2selection) {
            console.log(JSON.stringify($scope.minJS));
            var clusLink = "http://127.0.0.1:8089/phenotype/clusrmSettingsElements";
            var options = {
                url: clusLink,
                method: 'POST',
                params: {},
                data: {
                    ...
                    conditionw1selection: JSON.stringify($scope.conditionw1selection),
                    conditionw2selection: JSON.stringify($scope.conditionw2selection),
            ...
            $http(options)
                .success(function (data) {
                    ...
                    console.log(data.redescriptionsCLUS);
                    $scope.redescriptionsCLUS = data.redescriptionsCLUS;
                    $scope.descriptionsArray = data.descriptions;
                    $scope.maxAEJs = data.maxAEJs;
                    $scope.attJac = data.attJac;
                    $scope.AEJs = data.AEJs;
                    $scope.AAJs = data.AAJs;
                    $scope.sumAEJ = data.sumAEJ;
                    $scope.sumAAJ = data.sumAAJ;
                    $scope.EJsel = data.EJsel;
                    console.log(data.EJsel);
                    $scope.redescriptionTable = [];
                    for (i = 0; i < data.redescriptionsCountCLUS; i++) {
                        $scope.redescriptionTable[i] = {
                            redescription: $scope.redescriptionsCLUS[i],
    ...
    $scope.showSettingsClus = function () {
        console.log(this);
        if ($scope.showSettings == 1) {
            delete $scope.showSettings;
            $scope.buttonText = "Show settings";
        } else {
            $scope.showSettings = 1;
            $scope.buttonText = "Hide settings";
        }
    }

    $scope.showElements = function () {
        console.log(this.x.redescription);
        let id = this.x.redescription.id;
        $scope.descriptions = $scope.descriptionsArray[id - 1];
        for (i = 0; i < $scope.selSOMElements.length; i++) {
            id = "#" + $scope.selSOMElements[i].id;
            if ($(id).hasClass("match")) {
                $(id).removeClass("match");
                $(id).addClass("noMatch");
            }
        }
        for (i = 0; i < $scope.descriptions.length; i++) {
            id = "#" + $scope.descriptions[i].id;
            console.log($scope.descriptions[i].id);

            $(id).removeClass("noMatch");
            $(id).addClass("match");
        }
        let id2 = "#id" + this.x.redescription.id;
        for (i = 0; i < $scope.redescriptionTable.length; i++) {
            if (i == this.x.redescription.id)
                $(id2).addClass("selectedGray");
            else {
                id = "#id" + i;
                if ($(id).hasClass("selectedGray")) {
                    $(id).removeClass("selectedGray");
                    $(id).addClass("notSelected");
                } else $(id).addClass("notSelected");
            }
        }
    }
    ...
    function removeElementsAttrs() {
        for (i = 0; i < $scope.elementsDescriptionSelected.length; i++) {
            id = "#" + $scope.elementsId[i];
            if ($(id).hasClass("match")) {
                $(id).removeClass("match");
                $(id).addClass("noMatch");
            }
    ...
    $scope.showAttrs = function () {
        console.log(this.x.redescription);
        let id = this.x.redescription.id;
        $scope.descriptionAndName = [];
        for (var i = 0; i < $scope.attributeDescr[id - 1].length; i++)
            $scope.descriptionAndName[i] = {
                attrName: $scope.redescriptionsCLUS[id - 1].attrs[i],
                attrDescription: $scope.attributeDescr[id - 1][i]
            };
        console.log($scope.descriptionAndName);
        let id2 = "#id" + this.x.redescription.id;
        for (i = 1; i <= $scope.redescriptionTable.length; i++) {
            if (i == this.x.redescription.id)
                $(id2).addClass("selectedGray");
            else {
                id = "#id" + i;
        ...
        if ($scope.loadedShowElem != 0) {
            console.log(this.x.redescription);
            let id = this.x.redescription.id;
            $scope.descriptions = this.x.redescription.covered;
            for (i = 0; i < $scope.elementsDescriptionSelected.length; i++) {
                id = "#" + $scope.elementsId[i];
                if ($(id).hasClass("match")) {
                    $(id).removeClass("match");
            ...
            for (i = 0; i < $scope.descriptions.length; i++) {
                id = "";
                for (j = 0; j < $scope.elementsDescriptionSelected.length; j++) {
                    if ($scope.elementsSelected[j].localeCompare($scope.descriptions[i]) == 0)
                        id = "#" + $scope.elementsId[j];
                }
                console.log($scope.descriptions[i] + id);
    ...
    $scope.saveCLUSRMRedescriptions = function () {
        var checkedRedescriptions = '';
        $scope.redescriptionTable.forEach(function (x) {

            if (x.selected) {
                if (checkedRedescriptions != '') {
                    checkedRedescriptions += " , ";
                }
                checkedRedescriptions += x.redescription.id;
            }
        });
        $scope.result = checkedRedescriptions;
        if (!$scope.result) window.alert('Select redescriptions to save');
        else {
            var clusLink = "http://127.0.0.1:8089/phenotype/saveCLUSRMRedescriptions";
            var options = {
                url: clusLink,
                method: 'POST',
                params: {},
                data: {
                    ...
                    redescriptionsCLUS: JSON.stringify($scope.redescriptionsCLUS),
                    sumAEJ: JSON.stringify($scope.sumAEJ),
                    sumAAJ: JSON.stringify($scope.sumAAJ),
                    AEJs: JSON.stringify($scope.AEJs),
                    AAJs: JSON.stringify($scope.AAJs)
                }
            };
            $http(options)
                .success(function (data) {
                    window.alert('Redescriptions: ' + $scope.result + ' saved successfully');
                    //$scope.allRedUrl = "http://127.0.0.1:8089/phenotype/allRedescriptions";
                    //$scope.compglInfo();
                    //window.location.reload();
                    $scope.result = "";
                    //location.reload(true);
    ...
    $scope.saveCLUSRMRedescriptionsAttrs = function () {
        var checkedRedescriptions = '';
        $scope.redescriptionTable.forEach(function (x) {
    ...
    $scope.compHeatmapSel = function (params) {//function to load and create heatmap on selected data
        ...
        console.log("selected: " + $scope.selectionReds);
            ...
            if (redIds.size() > 0) {
                ...
                var options1 = {
                    ...
                    data: {
                        ...
                        selectedAttrs: JSON.stringify($scope.selectedCLUSRM.clus)
                    }
                };
    ...
    $scope.compglInfo = function () {
        ...
        $scope.checked = false;
        console.log($scope.getAllRedescriptions);
    }
    ...
    $scope.compglInfoSelSh = function () {
        ...
        if ($scope.action.type == "save") {
            ...
            $q.all([$scope.encapsulateSharedGet(options)]).then(function () {
                ...
                //depending on the value of a checkbox do standard or non standard
                console.log('data loaded from shared storage!');
                console.log($scope);
                console.log('graph input attrs');
                console.log($scope.$parent.graphInputAttrs);
                console.log('graph input');
                console.log($scope.$parent.graphInput);
                ...
                if (cch == false) {
                    ...
                    //non-standard
                    if (typeof $scope.$parent.graphInputAttrs == 'undefined') {
                        ...
                        var promiseNR = $scope.$parent.generalGETRequest(optionsNR);


                        promiseNR.success(function (data) {
                            $scope.$parent.numReds = data;
                            $scope.$parent.error = '';
                        })
                            .error(function (data, status) {
                                if (status === 404) {
                                    $scope.$parent.error = 'REST server is offline!';
                                } else {
                            ...
                            console.log($scope.$parent.numReds);
                            console.log($scope.$parent.numReds.numReds[0].numReds);

                            var numReds = $scope.$parent.numReds.numReds[0].numReds;
                            ...
                            joinedPromise.then(function (values) {
                                ...
                                $scope.$parent.graphInputAttrs = inputForAlgoAttrs;
                                console.log('graphInputAttrs loaded');
                                //console.log($scope.$parent.graphInputAttrs);

                                if (typeof $scope.$parent.graphInput == 'undefined') {
                                    var optionsNR = {
                                        url: numredsLink,
                                        method: 'GET',
                                        params: {}
                                    ...
                                    promiseNR.success(function (data) {
                                        ...
                                        for (var parts = 0; parts < numIters; parts++) {
                                            ...
                                            if (endIndex > numReds)
                                                endIndex = numReds;
                                            promisesE.push(getInParts(startIndex, endIndex, numReds, $scope));
                                        }

                                        var joinedPromiseE = Q.all(promisesE);

                                        joinedPromiseE.then(function (values) {
                                            ...
                                            $scope.$parent.graphInput = inputForAlgo;
                                            console.log('Entity graph loaded');
                                            ...
                                            console.log($scope.AllReds.measureNames);

                                            for (var mes = 0; mes < $scope.AllReds.measureNames.length; mes++)
                                                if ($scope.AllReds.measureNames[mes].shortName == "AEJ")
                                                    indAEJ = mes;
                                                else if ($scope.AllReds.measureNames[mes].shortName == "AAJ")
                                                    indAAJ = mes;
                                            ...
                                            for (var vi = 0; vi < tmp.length; vi++) {
                                                sum = 0;
                                                sumAt = 0;
                                                for (var vi1 = 0; vi1 < tmp.length; vi1++) {
                                                    if (vi == vi1)
                                                        continue;
                                                    sum = sum + 1.0 - $scope.$parent.graphInput[tmp[vi].id + ""][tmp[vi1].id + ""];
                                                    sumAt = sumAt + 1.0 - $scope.$parent.graphInputAttrs[tmp[vi].id + ""][tmp[vi1].id + ""];
                                                ...
                                                tmp[vi]['col' + (indAEJ)] = sum;
                                                tmp[vi]['col' + (indAAJ)] = sumAt;
                                            }

                                            var mins = new Array(5);
                                            var maxs = new Array(5);

                                            for (var vi = 0; vi < tmp.length; vi++) {
                                                for (var vi1 = 1; vi1 < 6; vi1++)
                                                    if (vi == 0) {
                                                        mins[vi1 - 1] = tmp[vi]['col' + (vi1 + 2)];
                                                        maxs[vi1 - 1] = tmp[vi]['col' + (vi1 + 2)];
                                                    } else {
                                                        if (mins[vi1 - 1] > tmp[vi]['col' + (vi1 + 2)])
                                                            mins[vi1 - 1] = tmp[vi]['col' + (vi1 + 2)];
                                                        if (maxs[vi1 - 1] < tmp[vi]['col' + (vi1 + 2)])
                                                            maxs[vi1 - 1] = tmp[vi]['col' + (vi1 + 2)];
                                                    }
                                            }

                                            for (var vi = 0; vi < mins.length; vi++) {
                                                $scope.AllReds.minMax[0]['maxMeasure' + (vi + 1)] = maxs[vi];
                                                $scope.AllReds.minMax[0]['minMeasure' + (vi + 1)] = mins[vi];
                                            }

                                            console.log('computed reds');
                                            console.log(tmp);
                                            $scope.AllReds.redescriptions = tmp;
                                            $scope.checked = false;

                                            //add measure computation part

                                        });
                                    });
                                } else {
                                    //add measure computation here and modify reds
                                }
    ...
    $scope.trainSom = function (params) {//functions to be defined on a button press
        ...
        if (params != null) {
            ...
            var timer = $timeout(function () {
                $scope.progress = "Getting SOM parameters!";
            }, 0);
            ...
            var timer4 = $timeout(function () {
                $scope.progress = "Training SOM map!";
            }, 2000);
            ...
            var feat = new Array();
            var som;
            var trainElementsI = new HashSet();
            ...
            var timer6 = $timeout(function () {
                ...
                if (params.nIter == 'all' || params.nIter > elemIds.size())
                    params.nIter = elemIds.size();

                som = new Som({
                    features: feat,
                    iterationCount: params.nIter/*elemIds.size()*/,
                    initialLearningRate: params.lRate,
                    width: params.nCols,
                    height: params.nRows
                });
                ...
                //var ob1={};
                for (var eci = 0; eci < elemIds.size(); eci++) {
                    var ob1 = {};
                    ob1.name = elems[eci];
                    ob1.count = ElementRedescriptionMap.get(elems[eci]).size();
                    ElementCoverage.push(ob1);
                }

                var q = 0;
                var interval = $interval(SOMIteration, 10,/*/params.nIter*/elemIds.size());
                ...
                function SOMIteration() {
                    ...
                    if (q < params.nIter) {
                        ...
                        if (params.nIter == elemIds.size()) {
                            elemString = '' + elems[q];
                            trainElementsI.add(q);
                        } else {
                            while (true) {
                                rind = $scope.randomIntFromInterval(0, elemIds.size() - 1);
                                if (!trainElementsI.contains(rind))
                                    break;
                            }
                            elemString = '' + elems[rind];
                            trainElementsI.add(rind);
                        }

                        var elemReds;

                        if (params.nIter == elemIds.size())
                            elemReds = ElementRedescriptionMap.get(elems[q]);
                        else elemReds = ElementRedescriptionMap.get(elems[rind]);
                        ...
                        if (q < params.nIter - 1/*(elemIds.size()-1)*/)
                            $scope.progress = (Math.floor(/*elemIds.size()*/q / params.nIter * 100)) + "% complete!";
                        if (q == (elemIds.size() - 1))
                            $scope.progress = "100 % complete!";
                        var tt;
                        $scope.checked = false;
                        $scope.action = tt;
                    } else {
                        if (q == params.nIter)
                            $scope.progress = "Assigning entities to clusters!";
                        else
                            $scope.progress = (Math.floor((q - params.nIter) / (elemIds.size() - params.nIter) * 100)) + "% complete!";
                        if (q < elemIds.size()) {
                            for (var ide = q - params.nIter; ide < elems.length; ide++) {
                                if (trainElementsI.contains(ide))
                                    continue;

                                elemString = '' + elems[ide];
                                somInput = {};
                                var elemReds = ElementRedescriptionMap.get(elems[ide]);
                                for (var j = 0; j < TmpIdRedIdMap.keys().length; j++) {
                                    var propName = feat[j];
                                    if (elemReds.contains(TmpIdRedIdMap.get(j))) {
                                        somInput[propName] = 1;
                                ...
                                //add SOM predictions here
                                som.predict(elemString, somInput);
                                console.log("predicting: " + ide);
                                break;
                            }
                        }
                ...
                interval.then(function () {
                    ...
                    console.log("SOM training time " + (t1 - t0) + " milliseconds.");
                    ...
                    $q.all([encapsulateSSD(options)]).then(function () {
                            ...
                            var options1 = {
                                url: $scope.CLusterInfoURL,
                                method: 'GET',
                                params: {
                                    userId: JSON.stringify($scope.userInfo.userInfo[0].userId)
                                }
                            };
                            $scope.getSOMClusterInfo(options1);
                        }
                    );
                });
            }, 2500);
    ...
    function initSettings() {
        ...
        $scope.numRetRed = 200;
        $scope.minSupport = 5;
        $scope.maxSupport = 120;
        $scope.minJS = 0.5;
        $scope.maxPval = 0.01;
        $scope.allowLeftNeg = "false";
        $scope.allowRightNeg = "false";
        $scope.allowLeftDisj = "false";
        $scope.allowRightDisj = "false";
    }
    ...
    $scope.trainOnSelection = function (params) {
        ...
        initSettings();
        if (typeof $scope.selectionReds == 'undefined') {
            alert('Please select a SOM cluster!');
            return;
        }

        if (typeof $scope.selectionReds.redescriptions == 'undefined') {
            alert('Please select a SOM cluster!');
            return;
        }

        if ($scope.selectionReds.redescriptions.length == 0) {
            alert('Please select a SOM cluster!');
            return;
        }
            ...
            var timer6 = $timeout(function () {
                ...
                function SOMIteration() {
                    ...
                    if (q < params.nIter) {
                        ...
                        if (q < params.nIter - 1/*(elemIds.size()-1)*/)
                            $scope.progress = (Math.floor(q / params.nIter * 100)) + "% complete!";
                ...
                interval.then(function () {
                    ...
                    $q.all([encapsulateSSD(options)]).then(function () {
                            ...
                            var options1 = {
                                url: $scope.CLusterInfoSelURL/*$scope.SOMURL*/,
                                method: 'GET',
                                params: {
                                    userId: JSON.stringify($scope.userInfo.userInfo[0].userId)
                                }
    ...
    $scope.trainOnSelectionShared = function (params) {//add code loading/saving to shared storage
        ...
        if ($scope.action.type == "save") {
            var timer4S = $timeout(function () {
                if ($scope.action.type == "save") $scope.progress = "Saving shared data!";
            });
            var minfr = 0;
            ...
            if (redIds.size() > 0) {
                ...
                var options1 = {
                    ...
                    data: {
                        userId: JSON.stringify($scope.userInfo.userInfo[0].userId),
                        sharedData: JSON.stringify(res1),
                        selectedAttrs: JSON.stringify([]),
                        selectedElems: JSON.stringify($scope.selSOMElements)
                    }
                };
                    ...
                    if (typeof $scope.SOMInput == 'undefined') {
                        ...
                        var options = {
                            ...
                            data: {
                                ...
                                sharedData: JSON.stringify(res),
                                selectedAttrs: JSON.stringify([]),
                                selectedElems: JSON.stringify($scope.RedsInSOMClust)
                            }
                        };
		...
		$scope.params=null;
		$scope.progress="Parameters cleared!";

		console.log("data in a button: ");
		console.log($scope.SOMInput);},2000);*/

            //var timer3=$timeout(function(){console.log("setting params to the old value!"); $scope.params=params; $scope.progress="Parameters restored!";},3000);
            var timer4 = $timeout(function () {
                if ($scope.action.type == "save") $scope.progress = "Saving shared data!"; else if ($scope.action.type == "load") $scope.progress = "Training SOM map!";
            }, 2000);
            ...
            var timer6 = $timeout(function () {
                ...
                if ($scope.action.type == "load") {
                    ...
                    var options1 = {
                        url: "http://127.0.0.1:8089/phenotype/redescriptionElement",
                        method: 'GET',
                        params: {}
                    };
                    ...
                    $q.all([$scope.encapsulateSharedGet(options), $scope.encapsulateGRE(options1)]).then(function () {
                        ...
                        function SOMIteration() {
                            ...
                            if (q < params.nIter) {
                                ...
                                var elemReds;
                                var rind = $scope.randomIntFromInterval(0, elemIds.size() - 1);
            ...
            if ($scope.action.type == "saveLayout") {
                ...
                var options1 = {
                    url: $scope.CLusterInfoSelBackURL/*$scope.SOMURL*/,
                    method: 'GET',
                    params: {}
                };
...
redApp.directive('drawGraphe', function () {
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                scope.$watch('RedRelationGraphE', function (newData, oldData) {
                    ...
                    /*if(newData.nodes.length>4 && newData.nodes.length<6)
					ld=Math.floor(2000/newData.nodes.length);
				else if(newData.nodes.length<=3) ld=Math.floor(1000/newData.nodes.length);
				else if(newData.nodes.length>6) ld=Math.floor(4000/newData.nodes.length);*/
                    ...
                    var links = svg.selectAll(".link")
                        ...
                        .style("stroke-width", function (d) {
                            return Math.sqrt(10 + (1.0 - d.value) * 10);
                        })
                        .style("stroke", function (d) {
                            if (d.group > 0) return color(d.group); else return "#000000";
                        });

                    var startNodeColor = '#009933', endNodeColor = '#ff0000', intermediaryNodesColor = '#33ccff',
                        regularNodesColor = '#808080';
                    ...
                    var node = svg.selectAll(".node")
                        ...
                        .style("fill", function (d) {
                            if (d.id == newData.startNode) return startNodeColor; else if (d.id == newData.endNode) return endNodeColor; else if (newData.intermediary.contains(d.id)) return intermediaryNodesColor; else return regularNodesColor;
                        })
                        .call(force.drag);
		...
		//console.log(scope.RedescriptionInfo);
                    ...
                    links.append("title")
                        .text(function (d) {
                            if (d.group > 0) {
                                return 'Path rank: ' + d.group + "." + '\nStrength: ' + (1.0 - d.value);
                            } else {
                                return 'Strength: ' + (1.0 - d.value);
                            }
                        });
            ...
            scope.$parent.$watch('Trends', function (newData, oldData) {
                ...
                for (j = 0; j < scope.$parent.SelectedRedInfo.sri.AttributeIntervals.length; j++) {
                    ...
                    if (scope.$parent.SelectedRedInfo.sri.AttributeIntervals[j].data[1] != "null" && scope.$parent.SelectedRedInfo.sri.AttributeIntervals[j].data[2] != "null") {
                        ...
                        var boxplotValues = [], attributeValuesAll = [], attributeValuesInterval = [],
                            attributeValuesRed = [], attributeValuesClause = [], attributeValuesAllAttrOcc = [],
                            boxplotValuesNew = [];
                        min = Infinity,
                            max = -Infinity;
                        ...
                        for (i = 0; i < scope.$parent.SelectedRedInfo.sri.RedAttributes.length; i++) {
                            ...
                            if (scope.$parent.SelectedRedInfo.sri.RedAttributes[i].id == scope.$parent.SelectedRedInfo.sri.AttributeIntervals[j].data[0]) {
                                ...
                                boxplotValuesNew.push({
                                    group: "All",
                                    value: scope.$parent.SelectedRedInfo.sri.RedAttributes[i].data[1]
                                });
                                var tmp = boxplotValuesNew[boxplotValuesNew.length - 1];
                                boxplotValuesNew[boxplotValuesNew.length - 1] = boxplotValuesNew[0];
                                boxplotValuesNew[0] = tmp;
                                ...
                                if (scope.$parent.SelectedRedInfo.sri.RedAttributes[i].data[1] >= scope.$parent.SelectedRedInfo.sri.AttributeIntervals[j].data[1] && scope.$parent.SelectedRedInfo.sri.RedAttributes[i].data[1] <= scope.$parent.SelectedRedInfo.sri.AttributeIntervals[j].data[2] && scope.$parent.SelectedRedInfo.sri.AttributeIntervals[j].data[3] == 0) {//add flag for negation
                                    ...
                                    var tmp = boxplotValuesNew[boxplotValuesNew.length - 1];
                                    if (boxplotValuesNew.length > 1) {
                                        boxplotValuesNew[boxplotValuesNew.length - 1] = boxplotValuesNew[1];
                                        boxplotValuesNew[1] = tmp;
                                    }
                                ...
                                for (k = 0; k < scope.$parent.SelectedRedInfo.sri.RedSupport.length; k++) {
                                    if (scope.$parent.SelectedRedInfo.sri.RedSupport[k].element == scope.$parent.SelectedRedInfo.sri.RedAttributes[i].data[0]) {
                                        ...
                                        var tmp = boxplotValuesNew[boxplotValuesNew.length - 1];
                                        if (boxplotValuesNew.length > 2) {
                                            boxplotValuesNew[boxplotValuesNew.length - 1] = boxplotValuesNew[2];
                                            boxplotValuesNew[2] = tmp;
                                        }
                                        intervalOK = 1;
                                        break;
                                    }
                                ...
                                if (intervalOK == 1) {
                                    ...
                                    if (containedUTmp == 1) {
                                        if (!avallSet.contains(scope.$parent.SelectedRedInfo.sri.RedAttributes[i].data[0])) {
                                            ...
                                            if (boxplotValuesNew.length > 3) {
                                                var tmp = boxplotValuesNew[boxplotValuesNew.length - 1];
                                                boxplotValuesNew[boxplotValuesNew.length - 1] = boxplotValuesNew[3];
                                                boxplotValuesNew[3] = tmp;
                                            }
                                        }
                                    }
                                    //  }
                                    ...
                                    if (contained == 1) {
                                        ...
                                        if (boxplotValuesNew.length > 4) {
                                            var tmp = boxplotValuesNew[boxplotValuesNew.length - 1];
                                            boxplotValuesNew[boxplotValuesNew.length - 1] = boxplotValuesNew[4];
                                            boxplotValuesNew[4] = tmp;
                                        }
                                    }
                        ...
                        boxplotValuesNew.forEach(function (d) {
                            d.value = +d.value;
                        });
                        ...
                        if ((scope.$parent.plotType == 1 || scope.$parent.plotType == 2) && scope.$parent.resetPlots == 0) {
                            chartDistAnalysis1.violinPlots.hide();
                            chartDistAnalysis1.notchBoxes.show({reset: true});
                            chartDistAnalysis1.boxPlots.show({
                                reset: true,
                                showBox: false,
                                showOutliers: true,
                                boxWidth: 20,
                ...
                if (scope.$parent.SelectedRedInfo.sri.ElemDesc.length < 100000) {
                    ...
                    var cells = rows.selectAll("td")
                        ...
                        .style("background-color", function (d, i) {
                            if ($.inArray(d.value, somClustElem) != -1) return "#4CAF50";
                        });//add array of SOM elements
                    ...
                    if (scope.$parent.SelectedRedInfo1.sri.AttributeIntervals[j].data[1] != "null" && scope.$parent.SelectedRedInfo1.sri.AttributeIntervals[j].data[2] != "null") {
                        ...
                        for (i = 0; i < scope.$parent.SelectedRedInfo1.sri.RedAttributes.length; i++) {
                            ...
                            if (scope.$parent.SelectedRedInfo1.sri.RedAttributes[i].id == scope.$parent.SelectedRedInfo1.sri.AttributeIntervals[j].data[0]) {
                                ...
                                boxplotValuesNew.push({
                                    group: "All",
                                    value: scope.$parent.SelectedRedInfo1.sri.RedAttributes[i].data[1]
                                });
                                var tmp = boxplotValuesNew[boxplotValuesNew.length - 1];
                                boxplotValuesNew[boxplotValuesNew.length - 1] = boxplotValuesNew[0];
                                boxplotValuesNew[0] = tmp;
                    ...
                    if (scope.$parent.SelectedRedInfo2.sri.AttributeIntervals[j].data[1] != "null" && scope.$parent.SelectedRedInfo2.sri.AttributeIntervals[j].data[2] != "null") {
                        ...
                        for (i = 0; i < scope.$parent.SelectedRedInfo2.sri.RedAttributes.length; i++) {
                            ...
                            if (scope.$parent.SelectedRedInfo2.sri.RedAttributes[i].id == scope.$parent.SelectedRedInfo2.sri.AttributeIntervals[j].data[0]) {
                                ...
                                boxplotValuesNew.push({
                                    group: "All",
                                    value: scope.$parent.SelectedRedInfo2.sri.RedAttributes[i].data[1]
                                });
                                var tmp = boxplotValuesNew[boxplotValuesNew.length - 1];
                                boxplotValuesNew[boxplotValuesNew.length - 1] = boxplotValuesNew[0];
                                boxplotValuesNew[0] = tmp;
...
//);
                            ...
                            if (newData.RedAttributes[i].id == newData.AttributeIntervals[j].data[0]) {
                                ...
                                for (k = 0; k < newData.RedSupport.length; k++) {
                                    if (newData.RedSupport[k].element == newData.RedAttributes[i].data[0]) {
                                        ...
                                        boxplotValuesNew.push({
                                            group: "Support",
                                            value: newData.RedAttributes[i].data[1]
                                        });
                                        var tmp = boxplotValuesNew[boxplotValuesNew.length - 1];
                                        if (boxplotValuesNew.length > 2) {
                                            boxplotValuesNew[boxplotValuesNew.length - 1] = boxplotValuesNew[2];
                                ...
                                if (intervalOK == 1) {
                                    ...
                                    //for(azat=0;azat<newData.AttributeIntervals.length;azat++){
                                    var attrsViewNew = 0;
                                    //if(newData.AttributeIntervals[j].data[4]==newData.AttributeIntervals[azat].data[4]){
                                    ...
                                    // }
                                    //else if(newData.AttributeIntervals[j].data[0] == newData.AttributeIntervals[azat].data[0]){
                                    /*for(azat=0;azat<newData.AttributeIntervals.length;azat++){
										  var attrsViewNew2=0;

										  for(iz=0;iz<newData.AttrDesc.length;iz++){
												if(newData.AttrDesc[iz].id==newData.AttributeIntervals[azat].data[0]){
														attrsViewNew2=newData.AttrDesc[iz].data[2];
															break;
													}
												}
											*/
                                    ...
                                    for (azat1 = 0; azat1 < newData.AttributeIntervals.length; azat1++) {
                                        ...
                                        /*var attrsViewTmp=0;
												...
												for(iz=0;iz<newData.AttrDesc.length;iz++){
												if(newData.AttrDesc[iz].id==newData.AttributeIntervals[azat1].data[0]){
														attrsViewTmp=newData.AttrDesc[iz].data[2];
															break;
													}
												}*/
                                        ...
                                        containedUTmp = 1;
                                        //if(attrsViewNew2==attrsViewTmp && attrsView==attrsViewTmp && newData.AttributeIntervals[azat1].data[4]==newData.AttributeIntervals[azat].data[4]){
			...
			<button ng-model="button" ng-disabled="checked" ng-click="trainOnSelectionShared(params)" style="margin-left: 80px;">Perform</button></div>
                        ...
                        /*for(var z=0; z<boxplotValues.length; z++){
													boxplotValues[z]=boxplotValues[z].sort(d3.ascending)
											var g=vis.append("svg:svg").attr("class","boxV").attr("width", w)
													.attr("height", h+30);

											var ttext=boxplotValues[z].length;

											  g.append("svg:g").attr("transform","translate ("+(m[3]) + "," + (m[0]+20) + ")").append("svg:text")
																.attr("x", 0).attr("y", 0).attr("font-family","sans-serif").attr("font-size","15px")
																.attr("fill","red").attr("align","center").text("("+ttext+")");

												if(boxplotValues[z].length>0){
											var gViolin=g.append("svg:g").attr("transform",  "translate(" + (m[3]-20) + "," + (m[0]+30) + ")");
											if(z==0)
												addViolin(gViolin, interpolation, 20, boxplotValues[z], h- m[0] - m[2] , w- m[1] - m[3]+40, domain, min, "#3385ff");//"#b780c9"
											else if(z==1)
												addViolin(gViolin, interpolation, 20, boxplotValues[z], h- m[0] - m[2] , w- m[1] - m[3]+40, domain, min, "#0099e6");
											else if(z==2)
												addViolin(gViolin, interpolation, 20, boxplotValues[z], h- m[0] - m[2] , w- m[1] - m[3]+40, domain, min, "#00e6e6");
											else if(z==3)
												addViolin(gViolin, interpolation, 20, boxplotValues[z], h- m[0] - m[2] , w- m[1] - m[3]+40, domain, min, "#a1dab4");
											else if(z==4)
												addViolin(gViolin, interpolation, 20, boxplotValues[z], h- m[0] - m[2] , w- m[1] - m[3]+40, domain, min, "#e6e600");
											addBoxPlot(gViolin, boxplotValues[z], h- m[0] - m[2], w- m[1] - m[3]+40, domain, .15, "black", "white");
												}
        ...
        link: function (scope, element, attrs) {
            ...
            scope.$watch('RedsInSOMClust', function (newData, oldData) {
                ...
                var STButton = divCont.append("button").attr("type", "button")
                    ...
                    .attr("title", "Create redescription relation graph.")
                    .style("width", "160px")
                    .style("margin-left", "20px")
                    .style("display", "inline-block")
                    .text('Relate redescriptions');

                divCont.append("p").attr("title", "Minimal allowed edge weight in the redescription relation graph.").style("margin-right", "0px").style("margin-left", "20px").style("display", "inline-block").append("text").text("Minimal weight: ");
                ...
                divCont.append("p").attr("title", "Number of paths to be computed connecting a selected sequence of redescriptions.").style("margin-right", "0px").style("margin-left", "20px").style("display", "inline-block").append("text").text("Num paths: ");
			...
			.data(function(row) {
				...
				return columns.map(function(column) {
					...
					if(column=="Left query")
						return {column: column, value: row.data[0]};
					else if(column=="Right query")
						return {column: column, value: row.data[1]};
					else if(column=="J"){
						//console.log("JS: "+row.data[2]);
						//console.log("JS round: "+row.data[2].toPrecision(2));
						return {column: column, value: row.data[2].toPrecision(2)};
					}
					else if(column=="support")
						return {column: column, value: row.data[3]};
					else if(column=="p-value")
						return {column: column, value: row.data[4].toPrecision(2)};
				});
			})*/
                var cells = rows.selectAll("td")
                    .data(function (row) {
                        ...
                        return columns.map(function (column) {
                            ...
                            if (column == "Left query")
                                ...
                                return {
                                    column: column,
                                    value: "<input type=\"checkbox\"  id=" + row.id + "E" + " class=\"rounded\"/>",
                                    id: row.id
                                };
                            }
                        });
                    ...
                    .html(function (d, i) {
						...
						if(scope.$parent.checkedOrder.co[k]==d.id){
									...
									console.log("checkbox unchecked!");
												console.log(scope.$parent.checkedOrder.co); break;}
													if(contained==0){
                ...
                var exButton = d3.select("#somContainer").append("button").attr("type", "button")
                    ...
                    .attr("title", "Export selected subset of redescriptions")
                    .style("margin-bottom", "20px")
                    .style("width", "150px")
                ...
                informCont.append("p").text('Max overlap (el.)').attr("title", "Maximal allowed entity overlap between any pair of redescriptions").style("width", "110px").style("position", "relative").style("float", "left");
                ...
                informCont.append("p").text('Max overlap (at.)').attr("title", "Maximal allowed attribute overlap between any pair of redescriptions").style("width", "110px").style("position", "relative").style("float", "left").style("margin-left", "10px");
                ...
                var RMRedButton = informCont.append("button").attr("type", "button")
                    ...
                    .attr("title", "Filter redundant redescriptions")
                    .style("width", "120px")
                    .style("margin-left", "20px")
        ...
        link: function (scope, element, attrs) {
            scope.$watch('RedsInSOMClust', function (newData, oldData) {
                ...
                textArray.forEach(function (d) {
                    ...
                    descString = descString.replaceAll(")", "");
                    descString = descString.replaceAll("n.e.s", "");
                    descString = descString.replaceAll("%", "");
                    descString = descString.replaceAll("+", "");
                    descString = descString.replaceAll(",", "");
                    descString = descString.replaceAll("'", "");
                ...
                tags.forEach(function (d) {
                    ...
                    //if(d[0].length>avg)
                    //d[1]=Math.round((d[1]/max)*200/(maxLength/avg));
                });
                ...
                /*var wnDiv= d3.select(element[0]).append("div")
					 .style("float","left")
                ...
                // wnDiv.append("p").text("The most frequent attributes occuring in redescriptions from the selected hexagon");
                ...
                WordCloud([canvas, htmlCanvas], {
                    list: tags.map(function (word) {
                        console.log('word');
                        console.log(word);
                        return [word[0], Math.round(word[1] / (Math.max((maxLength / 4), 1)))]
                    })
                });//{list:list} /3 deff
            });
        }
...
//console.log(newData.SOMInfo[0].data)
                ...
                for (i = 0; i </*newData.SOMInfo.length*/newData.SOMDim[0].nColumns * newData.SOMDim[0].nRows; i++) {
                    ...
                    /*if(typeof(newData.SOMInfo[i].data) != "undefined")
		SOMEntNum.push(newData.SOMInfo[i].data[0]);*/
                ...
                console.log(SOMConnectivity);
                console.log(SOMRedNum);
...
//Function to call when you mouseover a node
                function mover(d, i) {
                    ...
                    d3.select("#tooltip")
                        ...
                        .select("#value")
                        .text("Homogeneity: " + SOMConnectivity[i].toFixed(4));
    ...
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            scope.$watch('AllReds', function (newData, oldData) {
                ...
                /*	var jschartJS = chartCont.append("div")
							   .attr("id","js-chart")
							   .attr("class","chart")
							   .append("div")
							   .attr("class","title")
				...
				var jschartSupp = chartCont.append("div")
							   .attr("id","supports-chart")
							   .attr("class","chart")
							   .append("div")
							   .attr("class","title")
							   .text("Redescription support");

				var jschartpVal = chartCont.append("div")
							   .attr("id","pval-chart")
							   .attr("class","chart")
							   .append("div")
							   .attr("class","title")
				...
				var jschartAEJ = chartCont.append("div")
							   .attr("id","aej-chart")
							   .attr("class","chart")
							   .append("div")
							   .attr("class","title")
							   .text("Average element Jaccard similarity");

				var jschartpAAJ = chartCont.append("div")
							   .attr("id","aaj-chart")
							   .attr("class","chart")
							   .append("div")
							   .attr("class","title")
							   .text("Average atribute Jaccard similarity");*/
                ...
                /*var resd = d3.select(element[0]).append("div")
										.attr("id","results")
										.append("div")
										.append("div")
										.attr("class","title")
										.text("Most frequent attributes")
										.append("div")
										.attr("id","attrCount");*/
...
//d3.csv("repositories_data_119k.json", function(error, repos) {
	...
	fuse.list=workingData;*/
                ...
                // console.log(newData.minMax[0]);
                //console.log(newData.minMax[0][0]);
			...
			console.log(key);
                ...
                for (var key in newData.minMax[0]) {
                    ...
                    /*console.log(key);
			if(key.indexOf("max") > -1)
				console.log("contains max");
			else console.log("contains min");
			console.log(parseInt(key.match(/\d/g)));*/
                }
                //console.log(maxS);
                //console.log(minS);


                /*var arrayMeas = $.map(newData.minMax[0], function(value, index) {
			return [value];
	});
	...
	//console.log(arrayMeas);
	 ...
	 var iind=0;
	 for(i=0;i<arrayMeas.length;i++){
			//console.log(arrayMeas[i])
			if((iind%2)==0)
			maxS.push(arrayMeas[i]);
		else
			minS.push(arrayMeas[i]);
		 ...
		 iind++;
                ...
                /*for(j=0;j<newData.redescriptions.length;j++){
		for(i=2;i<newData.redescriptions.length;i++){
			if(newData.redescriptions[j].data[i]>maxS[i-2])
				maxS[i-2]=newData.redescriptions[j].data[i];
			if(newData.redescriptions[j].data[i]<minS[i-2])
				minS[i-2]=newData.redescriptions[j].data[i];
		 }
	 }*/
                ...
                /*jaccard = repo.dimension(function(d) {return d.data[2]; }),
      jaccards = jaccard.group(function(d) { return Math.floor(d*100); }),

      support = repo.dimension(function(d) { return d.data[3]; }),
      supports = support.group(function(d) { return Math.floor((d/199)*100); }),

      pval = repo.dimension(function(d) { return d.data[4]; }),
      pvals = pval.group(function(d) { console.log(Math.floor((-Math.log10(d)/17)*100 ));  if(d>0) return 100- Math.floor((-Math.log10(d)/17)*100 ); else return 100; }),

      aej = repo.dimension(function(d) { return d.data[5]; }),
      aejs= aej.group(function(d) { return Math.floor(d*100 ); });

	  aaj = repo.dimension(function(d) { return d.data[6]; }),
      aajs= aaj.group(function(d) { return Math.floor(d*100); });*/
                ...
                for (i = 2; i < newData.measures.length - 1; i++) {
                    ...
                    measureChart.push(repo.dimension(function (d) {
                        return d["col" + (i + 1)]/*d.data[i];*/
                    }));
                    measuresChart.push(measureChart[i - 2].group(function (d) { /*if(Math.floor(d/step)*step<minS[i-2]) return minS[i-2];*/
                        return Math.floor(d / step) * step;
                    }));
                    /*}
		  else{
			measureChart.push(repo.dimension(function(d) {return d.data[i]; }));
			measuresChart.push(measureChart[i-2].group(function(d) { return Math.floor(d/step)*step; }));*/
                    // }
                }

                /*jaccard = repo.dimension(function(d) {if(d.data[2]>maxJS) maxJS=d.data[2]; if(d.data[2]<minJS) minJS=d.data[2]; return d.data[2]; }),
      jaccards = jaccard.group(function(d) { return Math.floor(d*100)/100; }),

      support = repo.dimension(function(d) {if(d.data[3]>maxSupport) maxSupport=d.data[3]; if(d.data[3]<minSupport) minSupport=d.data[3]; return d.data[3]; }),
      supports = support.group(function(d) { return Math.floor(d/4)*4; }),

      pval = repo.dimension(function(d) { if(d.data[4]>maxPval) maxPval=d.data[4]; if(d.data[4]<minPval) minPval=d.data[4]; return d.data[4]; }),
      pvals = pval.group(function(d) {return Math.floor(d/0.0003)*0.0003;}),

      aej = repo.dimension(function(d) {if(d.data[5]>maxaej) maxaej=d.data[5]; if(d.data[5]<minaej) minaej=d.data[5]; return d.data[5]; }),
      aejs= aej.group(function(d) { return Math.floor(d*100 )/100; });

	  aaj = repo.dimension(function(d) {if(d.data[6]>maxaaj) maxaaj=d.data[6]; if(d.data[6]<minaaj) minaaj=d.data[6]; return d.data[6]; }),
      aajs= aaj.group(function(d) { return Math.floor(d*100)/100; });*/


                /*console.log(jaccard);
	  console.log(jaccards);
	  console.log("max/min JS");
	  console.log(maxJS);
	  console.log(minJS);
	   console.log("max/min pVal");
	  console.log(maxPval);
	  console.log(minPval);*/
        ...
        .group(jaccards)
      .x(d3.scale.linear()
        .domain([Math.max(minJS-0.1,0), maxJS+0.1])
        .rangeRound([0, 10 * 40])),
    ...
    barChart()
        ...
        .group(supports)
      .x(d3.scale.linear()
        .domain([Math.max(minSupport-5,0), maxSupport+5])
        .rangeRound([0, 10 * 40])),
    ...
    barChart()
        ...
        .group(pvals)
      .x(d3.scale.linear()
        .domain([Math.max(minPval-0.001,0), maxPval+0.001])
        .rangeRound([0, 10 * 40])),
     ...
     barChart()
        ...
        .group(aejs)
      .x(d3.scale.linear()
        .domain([Math.max(minaej-0.1,0), maxaej+0.1])
        .rangeRound([0, 10 * 40])),
     ...
     barChart()
        ...
        .group(aajs)
      .x(d3.scale.linear()
        .domain([Math.max(minaaj-0.1,0), maxaaj+0.1])
        .rangeRound([0, 10 * 40])),*/
                // ];
  ...
  var keys = [];
                ...
                function repoList(div) {
                    ...
                    div.each(function () {
                        ...
                        var red = d3.select(this).selectAll(".red")
                            .data(reposByJS, function (d) {/*console.log("list: "); console.log(d); console.log(d.id);*/
                                return d.id;
                            });
                        red.enter().append("div")
                            .attr("class", "red")
                        red.exit().remove();
                        /* red.on('click',function(d,i){
		  d3.selectAll('#selected').style('background-color', 'white');
		d3.selectAll('#selected').attr("id",null);
		d3.select(this).attr("id","selected");
		  var options = {url: attrs.redinfoUrl,
               method: 'GET',
               params: {
               id: JSON.stringify(reposByJS[i].id)
                        }
                          };
		scope.getRedescriptionInfo(options);
		//console.log(scope.RedescriptionInfo);
		  //console.log('Click on redescription: '+i); console.log('Redescription: '); console.log(reposByJS[i]);
		  });*/
                        ...
                        var repo = red.order().selectAll(".repo")
                            .data(function (d) {/*console.log("repo"); console.log(d);*/
                                return [d/*.data*/];
                            });
                        ...
                        repoEnter.append("div").attr("class", "selctR").append("input").attr("type", "checkbox").attr("id", function (d) {
                            return d.id
                        }).attr("class", "rounded");
                        ...
                        for (i = 0; i < newData.measures.length - 1; i++) {
                            if (i == 0) {
                                ...
                                repoEnter.append("div")
                                    .attr("class", "number")
                                    .text(function (d) {
                                        return parseFloat(d["col" + (i + 1)]).toFixed(2);
                                    });
                            } else if (i == 3) {
                                repoEnter.append("div")
                                    .attr("class", "number")
                                ...
                                repoEnter.append("div")
                                    .attr("class", "number")
                                    .text(function (d) {
                                        return parseFloat(d["col" + (i + 1)]).toFixed(4);
                                    });
                            } else {
                                repoEnter.append("div")
                                    .attr("class", "number")
      ...
      repoEnter.append("div")
          .attr("class", "number")
          .text(function(d) { return parseFloat(d[2]).toFixed(2); });
      repoEnter.append("div")
          .attr("class", "number")
          .text(function(d) { return d[3]; });
      repoEnter.append("div")
          .attr("class", "number")
          .text(function(d) { return parseFloat(d[4]).toFixed(4); });
      repoEnter.append("div")
          .attr("class", "number")
          .text(function(d) { return parseFloat(d[5]).toFixed(2); });
      repoEnter.append("div")
          .attr("class", "number")
          .text(function(d) { return parseFloat(d[6]).toFixed(2); });*/
                        repo.exit().remove();
                        repo.order();
                    });
                }
  ...
  $inputSearch.on('keyup', function(e){search2(e);});*/
                ...
                for (i = 2; i < newData.measures.length - 1; i++) {
                    nestG.push(d3.nest().key(function (d) {
                        return d["col" + (i + 1)];/*d.data[i];*/
                    }));
                    hjs.push($('#hcol' + (i + 1)));
                    ...
                    hjs[i - 2].on('click', function (e) {/*console.log(e);console.log(e.currentTarget.className);*/
                        var numb = parseInt(e.currentTarget.className.match(/\d/g)); /*console.log(numb);*/
                        var list1 = d3.selectAll(".list")
                            .data([function (d) {/*console.log("data function"); console.log(k);*/
                                return repoList1(d, nestG[numb - 3], measureChart[numb - 3]/*tmpnest,tmpMC*/);
                            }]);
                        list1.each(render);
                    });


                    //console.log(nestG[i-2]);
                    // console.log(hjs[i-2]);
                }

                /* var nestByJS = d3.nest().key(function(d) {console.log("nest"); console.log(d); return d.data[2]; });
  var $hjs=$('#hjs');
  $hjs.on('click',function(e){
      var list1 = d3.selectAll(".list")
      .data([function(d){return repoList1(d,nestByJS,jaccard);}]); list1.each(render);});

    var $hsupp=$('#hsupp');
  $hsupp.on('click',function(e){
      var nestBySupport = d3.nest().key(function(d) { return d.data[3]; });
      var list1 = d3.selectAll(".list")
      .data([function(d){return repoList1(d,nestBySupport,support);}]); list1.each(render);});

       var $hpval=$('#hpval');
  $hpval.on('click',function(e){
      var nestByPval = d3.nest().key(function(d) { return d.data[4]; });
      var list1 = d3.selectAll(".list")
      .data([function(d){return repoList1(d,nestByPval,pval);}]); list1.each(render);});

       var $hejs=$('#hejs');
  $hejs.on('click',function(e){
      var nestByLang = d3.nest().key(function(d) { return d.data[5]; });
      var list1 = d3.selectAll(".list")
      .data([function(d){return repoList1(d,nestByLang,aej);}]); list1.each(render);});

             var $hajs=$('#hajs');
  $hajs.on('click',function(e){
      var nestBySize = d3.nest().key(function(d) { return d.data[6]; });
      var list1 = d3.selectAll(".list")
      .data([function(d){return repoList1(d,nestBySize,aaj);}]); list1.each(render);});*/
                ...
                function repoList1(div, nest, filtData) {
                    ...
                    div.each(function () {
                        //console.log(reposBy);
                        var red = d3.select(this).selectAll(".red")
                            .data(passOn/*reposBy[0].values*/, function (d) {/*console.log("list1: "); console.log(d); console.log(d.id);*/
                                return d.id;
                            });
                        red.enter().append("div")
                            .attr("class", "red")
		...
		d3.select(this).attr("id","selected");
		  var options = {url: attrs.redinfoUrl,
               method: 'GET',
               params: {
               id: JSON.stringify(reposBy[0].values[i].id)
                        }
                          };
		scope.getRedescriptionInfo(options);
		//console.log(scope.RedescriptionInfo);
                        ...
                        for (i = 0; i < newData.measures.length - 1; i++) {
                            if (i == 0) {
                                repoEnter.append("div")
                                    .attr("class", "leftRule")
                                    .text(function (d) {/*console.log("LR"); console.log(d);*/
                                        return d["col" + (i + 1)];
                                    });
                            } else if (i == 1) {
                                repoEnter.append("div")
      ...
      repoEnter.append("div")
          ...
          .text(function(d) { return parseFloat(d[6]).toFixed(2); });*/
                ...
                /*var progBar=d3.select("#lists").append("div").attr("id","barContainer").attr("class","w3-progress-container w3-light-blue").style("visibility","hidden").style("margin-bottom","10px");
						progBar.append("div").attr("id","bar").attr("class","w3-progressbar w3-blue").style("width","0%").style("margin-bottom","10px").style("opacity","0.5").style("visibility","hidden");
						progBar.append("p").attr("id","barText").style("margin-left","50px").style("font-weight","bold").text("0% complete...");*/

                //add control buttons to relate redescriptions
...
redApp.directive('loadheatData', function ($q, $timeout) {
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                // scope.getHeatmapData(options);//create
		...
		var options1 = {url: attrs.attrfreqUrl,
               method: 'POST',
               params: {
                        },
				data: { attributeFrequency:  JSON.stringify(returnData),
						attCooc: JSON.stringify(scope.heatData.attCooc)}
                          };
		 ...
		 scope.$watch('heatData1', function(newData, oldData){
			 if(!newData){return;}
		 });*/
...
redApp.directive('attributeHeatmap', function () {
        ...
        link: function (scope, element, attrs) {
            ...
            scope.$watch('Coocurence', function (newData, oldData) {
                ...
                newData.nodes1.forEach(function (node, i) {
                    matrix[i] = d3.range(newData.nodes2.length).map(function (j) {
                        return {x: j, y: i, z: 0}
                    });
                });


                //console.log(map1); console.log(map2); console.log(matrix);
                // Convert links to matrix; count character occurrences.
                newData.links.forEach(function (link) {
                    //console.log("map 1"); console.log(map1.get(link.source));
                    //console.log("map 1"); console.log(map1.get(link.source));
                    matrix[map1.get(link.source)][map2.get(link.target)].z += link.value;
                    // matrix[map1.get(link.target)][map2.get(link.source)].z += link.value;
                    // matrix[map1.get(link.source)][map2.get(link.source)].z += link.value;
                    //matrix[map1.get(link.target)][map2.get(link.target)].z += link.value;
                ...
                var orders = {
                    name1: d3.range(newData.nodes1.length).sort(function (a, b) {
                        return d3.ascending(newData.nodes1[a].Name, newData.nodes1[b].Name);
                    }),
                    count1: d3.range(newData.nodes1.length).sort(function (a, b) {
                        return newData.nodes1[b].count - newData.nodes1[a].count;
                    }),
                    name2: d3.range(newData.nodes2.length).sort(function (a, b) {
                        return d3.ascending(newData.nodes2[a].Name, newData.nodes2[b].Name);
                    }),
                    count2: d3.range(newData.nodes2.length).sort(function (a, b) {
                        return newData.nodes2[b].count - newData.nodes2[a].count;
                    }),
                ...
                row.append("text")
                    ...
                    .text(function (d, i) {/*console.log(newData.nodes1[i].Name);*/
                        var ch = newData.nodes1[i].Name.match(/(.|[\r\n]){1,10}/g);
                        var retString = ch[0];
                        return retString/*newData.nodes1[i].Name;*/
                    });
                ...
                column.append("text")
                    ...
                    .text(function (d, i) {/*console.log(newData.nodes2[i].Name);*/
                        var ch = newData.nodes2[i].Name.match(/(.|[\r\n]){1,15}/g);
                        var retString = ch[0];
                        return retString; /*newData.nodes2[i].Name;*/
                    });
	...
	console.log(p);
	console.log("column: "+newData.nodes2[p.x].Name);
	console.log("row: "+newData.nodes1[p.y].Name);*/
                    ...
                    d3.select("#tooltipA")
                        ...
                        .select("#value")
                        .text(function () {
                            var ch = newData.nodes1[p.y].Description.match(/(.|[\r\n]){1,26}/g);
                            var res = "";
                            for (var it = 0; it < ch.length; it++) if (it + 1 < ch.length) res = res + ch[it] + "\n"; else res = res + ch[it];
                            return "Row: " + res;/*newData.nodes1[p.y].Description*/
                        })

                    d3.select("#tooltipA").select("#value1")
                        .text(function () {
                            var ch = newData.nodes2[p.x].Description.match(/(.|[\r\n]){1,26}/g);
                            var res = "";
                            for (var it = 0; it < ch.length; it++) if (it + 1 < ch.length) res = res + ch[it] + "\n"; else res = res + ch[it];
                            return "Column: " + res; /*"Column: "+newData.nodes2[p.x].Description*/
                        });
                    //Show the tooltip
                ...
                d3.select("#order").on("change", function () {
                    ...
                    scope.selectedCLUSRM.clus = [];
                    $("#selectedCLUSRM").hide();
                    $("#selectedCLUSRM1").hide();
                    console.log(scope.selectedCLUSRM.clus);
                    order(this.value);
                });
		  ...
		  console.log(matrix);*/
                    ...
                    t.selectAll(".column")
                        .delay(function (d, i) {/*console.log("i");console.log(i); console.log(y(i)); */
                            return y(i) * 4;
                        })
                        .attr("transform", function (d, i) {
                            return "translate(" + y(i) + ")rotate(-90)";
	...
	*/
                ...
                var pagesRow = Math.ceil(newData.attcountW1[0].count / 50),
                    pagesCol = Math.ceil(newData.attcountW2[0].count / 50);
                ...
                /*$("#pagingR").pagination("onPageClick",function(){
			  ...
			  console.log(p1); console.log(p2);
                ...
                /*var exButton = d3.select("#heatmap").append("button").attr("type","button")
										.attr("id","exButt")
										.attr("onclick","exportFunction()")
										.style("margin-bottom","20px")
										.text('Export redescriptions');*/
...
redApp.directive('attributeRedescriptions', function ($compile) {
  ...
  //var scope = angular.element($("#outer")).scope();
    scope.$apply(function(){
					...
					if(scope.RedsContainingAttrs.redescriptions[1].id==81){
						scope.RedsContainingAttrs.redescriptions.splice(0,1);
					}
		});
		}*/
            ...
            scope.$watch('RedsContainingAttrs', function (newData, oldData) {
                ...
                for (var i = 0; i < newData.redescriptions.length; i++) {
                    ...
                    checkMap.set(newData.redescriptions[i].id, new HashSet());
                }
                ...
                for (var i = 0; i < newData.redescriptionAttrs.length; i++) {
                    ...
                    if (checkMap.get(newData.redescriptionAttrs[i].id).contains(newData.redescriptionAttrs[i].data[0]))
                        continue;
                    else checkMap.get(newData.redescriptionAttrs[i].id).add(newData.redescriptionAttrs[i].data[0]);
                ...
                if (scope.selectedCLUSRM.clus.length != scope.selectionNumber * 2) {
                    scope.selectedCLUSRM.clus.splice(scope.selectedCLUSRM.clus.length - 2, 2);
                    delete scope.noRedescriptionsfound;
                    delete scope.redescriptionsCLUS;
                    delete scope.showSettings;
                    scope.buttonText = "Show settings";
                }
                scope.selectedCLUSRM.clus.push(newData.selAttrs[1].attributeName);
                ...
                var cells = rows.selectAll("td")
                    ...
                    .attr("style", "text-align:center")
                    .attr("width", function (d, i) {
                        if (i == 0) return "10%"; else return "45%";
                    });
                ...
                scope.selectedCLUSRM.clus.push(newData.selAttrs[0].attributeName);
                $("#selectedCLUSRM").show();
                $("#selectedCLUSRM1").show();
                initSettings();
...
//add here back button and text
                ...
                /*attr("id","barContainer").attr("class","w3-progress-container w3-light-blue").style("visibility","hidden").style("margin-bottom","10px");
						progBar.append("div").attr("id","bar").attr("class","w3-progressbar w3-blue").style("width","0%").style("margin-bottom","10px").style("opacity","0.5").style("visibility","hidden");
						progBar.append("p").attr("id","barText").style("margin-left","50px").style("font-weight","bold").text("0% complete...");
		*/
                GlScope = scope;
                //element.bind('RMRedButton',filterRedes);
                //scope.filterRedsc=filterReds;
                ...
                //scope.selectedCLUSRM = "";
                console.log(newData);
                if (scope.loadedAttr == 1) {
                    scope.loadedShowElem = 1;
                    scope.elementsDescriptionSelected = newData.elementsDescriptionSelected;
                    scope.elementsId = [];
                    for (i = 0; i < scope.elementsDescriptionSelected.length; i++) {
                        scope.elementsId.push(scope.elementsDescriptionSelected[i].replace(/[^A-Za-z0-9]/g, ""));
                        console.log(scope.elementsId[i]);
                    }
                    scope.elementsSelected = newData.elementsSelected;
                } else scope.loadedShowElem = 0;
                for (i = 0; i < newData.selAttrs.length; i++) {
                    console.log(newData.selAttrs[i].attributeName);
                    /*if(scope.selectedCLUSRM.split(";").length != scope.selectionNumber*2 || (scope.selectionNumber==0 && scope.selectedCLUSRM == ""))
            scope.selectedCLUSRM = newData.selAttrs[i].attributeName;
        else if(scope.selectedCLUSRM.split(";").length == scope.selectionNumber*2)
            scope.selectedCLUSRM += ";" + newData.selAttrs[i].attributeName;*/
                }

                /*var tables = d3.select("#heatmap").append("div").attr("id","glAt1Tab");
	tables.style("position","relative").style("margin-bottom","5px").style("height","100px").style("overflow-y","scroll").style("width","50px").style("float","left");
	tables.append("table").attr("id","glAt1").attr("class","tablesorter").attr("border-collapse","separate").attr("border-spacing","10 10").attr("border","1px solid black").style("position","relative")
	.style("height","100px").style("margin-left","0px");*/
                    ...
                    .data(function (row) {
                        ...
                        return columns.map(function (column) {
                            ...
                            if (column == "Left query")
                                ...
                                return {
                                    column: column,
                                    value: "<input type=\"checkbox\"  id=" + row.id + "A" + " class=\"rounded\"/>",
                                    id: row.id
                                };
                            }
                        });
				...
				if(i==0){
					//console.log("d");
				//console.log(d);
					var tArr=d.value.split('=');
					//console.log("tArr");
				//	console.log(tArr);
					 var num= tArr[2].split(" ");
					idT=parseInt(num[0]);
				//	console.log(idT);
					return idT;
				}
